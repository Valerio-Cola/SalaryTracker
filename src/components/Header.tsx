    ...
    <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-30 shadow-xs transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row items-center justify-between py-3 gap-3">
          ...
          {/* Selettore Mese con Popover Calendario Mese/Anno */}
          <div className="relative flex items-center justify-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700 w-full sm:w-auto transition-colors">
            <button
              onClick={handlePrevMonth}
              className="p-1.5 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white hover:shadow-xs transition-all shrink-0"
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1 hover:bg-white dark:hover:bg-slate-700/70 rounded-lg transition-colors cursor-pointer group"
              title="Apri selettore Mese e Anno"
            >
              <Calendar className="w-4 h-4 text-blue-600 dark:text-blue-400 shrink-0" />
              <span className="text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-200 capitalize">
                {formattedMonth}
              </span>
              <ChevronDown className={`w-3.5 h-3.5 text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-200 transition-transform ${isPickerOpen ? 'rotate-180 text-blue-600' : ''}`} />
            </button>

            <button
              onClick={handleCurrentMonth}
              className="px-2 py-1 text-[11px] sm:text-xs font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/60 hover:bg-blue-100 dark:hover:bg-blue-900/60 rounded-lg transition-colors shrink-0"
